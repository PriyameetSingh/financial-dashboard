...
export function SearchableKpiUserField({
  ...
  return (
    <div ref={rootRef} className="block text-xs uppercase tracking-[0.3em] text-[var(--ax-muted)]">
      ...
      <div className="relative mt-2">
        <button
          ...
          className="flex w-full items-center justify-between gap-2 rounded-xl border border-[var(--color-divider)] bg-[var(--color-surface)] px-3 py-2 text-left text-sm font-normal normal-case tracking-normal text-[var(--color-text)] disabled:opacity-50"
        >
          <span className="min-w-0 truncate">
            {selected ? formatUserLine(selected) : <span className="text-[var(--ax-muted)]">Select user…</span>}
          </span>
          <span className="shrink-0 text-[var(--ax-muted)]" aria-hidden>
            ▾
          </span>
        </button>
        {open && !disabled && (
          <div
            className="absolute left-0 right-0 z-10 mt-1 rounded-xl border border-[var(--color-divider)] bg-[var(--color-surface)] p-2 shadow-lg"
            role="listbox"
          >
            <label className="mb-2 block text-[10px] font-normal normal-case tracking-normal text-[var(--ax-muted)]">
              Search
              <input
                type="search"
                value={query}
                ...
                className="mt-1 w-full rounded-lg border border-[var(--color-divider)] bg-[var(--color-surface)] px-2 py-1.5 text-sm font-normal normal-case tracking-normal text-[var(--color-text)] placeholder:text-[var(--ax-muted)]"
                autoFocus
              />
            </label>
            <ul className="max-h-48 overflow-y-auto text-sm font-normal normal-case tracking-normal">
              <li>
                <button
                  ...
                  role="option"
                  aria-selected={value === ""}
                  className="w-full rounded-lg px-2 py-1.5 text-left text-[var(--ax-muted)] hover:bg-[var(--ax-hover)]"
                  onClick={() => {
                    onChange("");
                    setOpen(false);
                    setQuery("");
              ...
              {filtered.length === 0 ? (
                <li className="px-2 py-2 text-[var(--ax-muted)] normal-case">No matches.</li>
              ) : (
                filtered.map((u) => {
                  ...
                  return (
                    <li key={u.id}>
                      <button
                        ...
                        className="w-full rounded-lg px-2 py-1.5 text-left text-[var(--color-text)] hover:bg-[var(--ax-hover)] disabled:cursor-not-allowed disabled:opacity-50"
                        onClick={() => {
                          if (excluded) return;
                          onChange(u.id);
                          setOpen(false);
                      ...
                      >
                        {formatUserLine(u)}
                        <span className="mt-0.5 block text-[11px] lowercase text-[var(--ax-muted)]">{u.email}</span>
                      </button>
                    </li>
                  );
                })
    ...
    >
      ...
      >
        <div className="mb-4 flex items-start justify-between gap-4">
          <div>
            <h2 id="add-kpi-modal-title" className="text-lg font-semibold text-[var(--color-text)]">
              Add KPI
            </h2>
        ...
        <div className="space-y-3">
          ...
          {/* <div className="flex justify between">
            <p className="text-xs uppercase tracking-[0.3em] text-[var(--ax-muted)]">Category</p>
            <span className="mt-2 inline-block rounded-lg border border-[var(--color-divider)] bg-[var(--color-surface)] px-3 py-1.5 text-sm font-medium text-[var(--color-text)]">
              {derivedCategory}
            </span>
          </div> */}
          ...
          <label className="block text-xs uppercase tracking-[0.3em] text-[var(--ax-muted)]">
            ...
            <select
              ...
              className="mt-2 w-full rounded-xl border border-[var(--color-divider)] bg-[var(--color-surface)] px-3 py-2 text-sm font-normal normal-case tracking-normal text-[var(--color-text)]"
            >
          ...
          <div>
            <p className="text-xs uppercase tracking-[0.3em] text-[var(--ax-muted)]">KPI Type</p>
                ...
                return (
                  <button
                    ...
                    className={`rounded-xl border px-3 py-2.5 text-left transition-colors ${
                      selected
                        ? "border-[var(--color-text)] bg-[var(--ax-hover)]"
                        : "border-[var(--color-divider)] bg-[var(--color-surface)] hover:bg-[var(--ax-hover)]"
                    }`}
                  >
                    <p className={`text-xs font-semibold uppercase tracking-widest ${selected ? "text-[var(--color-text)]" : "text-[var(--ax-muted)]"}`}>
                      {meta.label}
                    </p>
                    <p className="mt-1 text-[11px] font-normal normal-case tracking-normal text-[var(--ax-muted)] leading-snug">
                      {meta.description}
                    </p>
                  </button>
                );
          ...
          {kpiType === "OUTCOME" && (
            <p className="rounded-xl border border-[var(--color-divider)] bg-[var(--color-surface)] px-3 py-2.5 text-[11px] normal-case tracking-normal text-[var(--ax-muted)]">
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="mt-1 text-xs text-[var(--ax-muted)]">Anyone listed may enter or update KPI progress.</p>
              <button
                type="button"
              ...
              <div className="flex items-center gap-2 pt-2 border-t border-[var(--color-divider)]">
                ...
                />
                <label htmlFor="kpi-self-approve-checkbox" className="text-xs uppercase tracking-[0.1em] text-[var(--ax-muted)] cursor-pointer select-none">
          ...
          {kpiType === "OUTPUT" && (
            <div className="rounded-xl border border-[var(--color-divider)] bg-[var(--color-surface)] p-3 space-y-3">
              <p className="text-xs uppercase tracking-[0.3em] text-[var(--ax-muted)]">Measurement</p>
              ...
              <div className="flex items-center gap-2 pt-1">
                <input
                  ...
                  className="h-4 w-4 rounded border-[var(--color-divider)] bg-[var(--color-surface)] focus:ring-[var(--color-accent)]"
                />
                <label htmlFor="different-units-checkbox-add" className="text-xs uppercase tracking-[0.15em] text-[var(--ax-muted)] cursor-pointer select-none">
              ...
              <div className="grid gap-3 md:grid-cols-2">
                ...
                <label className="block text-xs uppercase tracking-[0.3em] text-[var(--ax-muted)] col-span-1">
                  Target
                  <span className="mt-0.5 block text-[10px] font-normal normal-case tracking-normal text-[var(--ax-muted)] opacity-80">
              ...
              {(unit || denominatorUnit) && denominator && (
                <p className="text-[11px] text-[var(--ax-muted)] normal-case tracking-normal">
