 ...
 *
 * Plain JS and dependency-free (node:zlib only) so BOTH the vitest helper
 * (tests/helpers/pdf-text.ts re-exports it) and the golden HTTP smoke leg
 * (scripts/check-http-smoke.mjs) share ONE implementation. Two copies of a
 * text extractor would be two chances to "prove" a header present that is not.
 *
