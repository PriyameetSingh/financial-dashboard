...
async function main() {
  ...
  for (const sb of schemeBudgets) {
    ...
    // Scheme-level budget: no subscheme. Prisma disallows null in compound-unique `where` for upsert,
    // so use findFirst + update/create instead.
    ...
    if (existingBudget) {
      await prisma.financeBudget.update({
        ...
        data: { budgetEstimateCr: sb.budgetCr },
      });
    } else {
  ...
  for (const topic of topics61) {
    await prisma.meetingTopic.create({ data: tenantStamped({ meetingId: meeting61.id, topic }) });
  }
