  ...
  try {
    ...
    const rows = await prisma.user.findMany({
      ...
      select: {
        ...
        department: true,
        designationId: true,
        designationRel: { select: { name: true } },
        organisationId: true,
        organisationRel: { select: { name: true } },
        ulbId: true,
        ulbRel: { select: { name: true } },
        userSections: {
          include: {
    ...
    const users = rows
      ...
      .map((r) => {
        ...
        return {
          ...
          designation: r.designationRel?.name ?? "",
          designationId: r.designationId ?? null,
          organisation: r.organisationRel?.name ?? "",
          organisationId: r.organisationId ?? null,
          ulb: r.ulbRel?.name ?? "",
          ulbId: r.ulbId ?? null,
          sections: r.userSections.map((us) => ({ id: us.section.id, name: us.section.name })),
